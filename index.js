...
if (!fs.existsSync(CREDS)) {
  ...
  console.log("♻️ ☠️BEOWULF☠️-MD session restored successfully");
}
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || '☠️BEOWULF☠️'}</p>
        </div>
      </body>
      </html>
    `);
...
async function connectToWA() {
  ...
  console.log(`🤖 ☠️BEOWULF☠️-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ ☠️BEOWULF☠️-MD Bot connected to WhatsApp!");
      ...
      const aliveMsg = `*╭──────────────●●►*\n> *☠️BEOWULF☠️-MD CONNECTED SUCCESSFULLY*\n\n> *Type ${prefix}menu to view commands*  \n\n*╭⊱✫ ☠️BEOWULF☠️ MD ✫⊱╮*\n*│✫📂 Bot Name: ${botConfig.BOT_NAME}*\n*│✫🛡️ Owner: ${config.OWNER_NAME}*\n*│✫♻️ Prefix: ${prefix}*\n*│✫🌍 Mode: ${config.MODE}*\n*│✫⏰ Uptime: ${runtime(process.uptime())}*\n*╰──────────────●●►*\n\n> Enjoy Using ☠️BEOWULF☠️ MD`;
      
      // Image URL for connection message
      const imageUrl = 'https://kommodo.ai/i/7Pr3UGnYcoBXEzusbHv8;
      
      try {
        // Send to owner with image
        sock.sendMessage(ownerNumber[0] + '2348030559356@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
      ...
      for (const participant of participants) {
        ...
        if (action === 'add') {
          ...
          if (settings.goodbye) {
            try {
              // Get user's profile picture
              const ppUrl = await getProfilePicture(sock, participant).catch(() => 'https://kommodo.ai/i/7Pr3UGnYcoBXEzusbHv8');
